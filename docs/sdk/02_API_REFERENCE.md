...
- `axes`: **축별 정지토크** `{axis: {hold_enabled, hold_cs}}`
  ...
  설정된 축이 바로 통전됩니다.
- `axes[n].run_cs`: **축별 운전 토크** — 그 축이 **움직일 때**의 코일 전류 1-19.
  굽힘축은 토크가 필요하고 피드 롤러는 그렇지 않은데, 예전에는 셋이 한 값을
  공유했습니다. 응답의 `run_cs_effective` 가 **실제로 코일에 들어가는 값**입니다.
...
| 필드 | 뜻 |
|---|---|
| `cs_max` | **19** — 절대 상한. 2026-05-08에 CS=31로 보드 2장이 소손됨 |
| `cs_cap` | 현재 PSU 프리셋이 허용하는 상한 (`/api/system/psu`) |

- 20 이상 또는 0 이하는 **422로 거부**됩니다
- 상한 이내지만 PSU 캡을 넘는 요청은 거부가 아니라 **자동으로 깎입니다** —
  요청값 `run_cs`, 실제 적용값 `run_cs_effective` 로 각각 보고되어, 깎인 축을
  설정된 축으로 오해할 수 없습니다
- 상태 파일을 손으로 고쳐도 로드 시 다시 클램프됩니다

**예시 — 굽힘축만 토크를 올리고 피드는 낮추기**
```bash
