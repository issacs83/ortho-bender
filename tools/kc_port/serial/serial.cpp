...
size_t Serial::read (std::vector<uint8_t> &buffer, size_t size) {
  ...
  size_t bytes_read = 0;
  try { bytes_read = this->pimpl_->read (buffer_, size); }
...
size_t Serial::readline (string &buffer, size_t size, string eol) {
  ...
  while (true) {
    ...
    if (string (reinterpret_cast<const char*>(buffer_ + read_so_far - eol_len), eol_len) == eol) break;
    if (read_so_far == size) break;
  }
...
void Serial::flushInput () { ScopedReadLock lock(this->pimpl_); pimpl_->flushInput (); }
void Serial::flushOutput () { ScopedWriteLock lock(this->pimpl_); pimpl_->flushOutput (); }
